                    ...
                    <div class="table-responsive">
                        <div class="row">
                            ...
                            <table class="table zero-configuration dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                ...
                                <tbody>                      
                                @foreach($patient->reveals as $reveal)
                                <tr>
                                    <td>{{$reveal->status}}</td>
                                    <td>{{$reveal->detection_date}}</td>
                                    <td>
                                        @if (auth()->user()->hasPermission('read-reveals'))
                                            <a href="{{url(route('details', $reveal->id))}}" class="btn btn-primary"><i class="fa fa-eye"></i></a>
                                        @else
                                        <button type="submit" class="btn btn-primary btn-xs disabled"><i class="fa fa-edit"></i></button>
                                        @endif
                                        </td>
